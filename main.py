...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.project_path = os.path.join(self.baseDir, "project")

        self.date_string = datetime.now().strftime("%Y-%m-%d")
        self.dateText.setPlaceholderText(f"{self.date_string}")
    ...
    def setDirectoryEvent(self):
        try: 
            ...
            if directory.exec_() == QFileDialog.Accepted:
               ...
               self.streamDirectory.setPlaceholderText(f'{directory_path}')
               self.streamDirectory.setText(f"{directory_path}")
        except Exception as e:
            QMessageBox.information(self, "Invalid project folder", "No such local directory")
            return
    ...
    def saveInformationEvent(self):
        try:
            ...
            if self.location == "" or self.serialnum == "": raise Exception("Location and Serial Number is empty")
    
            self.textFilename = f"{self.location}_{self.serialnum}_info_{self.date_string}.txt" # Create a filename for the text
            ...
            with open(f"{self.directory}\{self.textFilename}", "w") as f:
                ...
                QMessageBox.information(self, "{} saved successfully".format(self.textFilename), "Check the current directory for the text file")
    ...
    def importButtonEvent(self):
        try:
            ...
            if self.fileType.currentIndex() == 0: self.open_video()
        ...
        try:
            ...
            if self.video_source:
                ...
                if duration <= 60:
                    ...
                    if self.paused == True: self.playPauseVideoEvent()
                    ...
                    QMessageBox.information(self, "Error", "The selected video exceeds 1 minute duration.")
        except Exception as e:
            QMessageBox.information(self, "Error", f"{e}")
            return
    ...
    def saveCapturedFrame(self):
        self.location = self.location_text_edit.toPlainText()
        self.serialnum = self.serialnum_text_edit.toPlainText()
        filename = "{}_{}_capture{}.png".format(self.location, self.serialnum, datetime.now().strftime("%Y-%m-%d-%H%M%S"))
    ...
    def load_model(self): #load model CPU only
        try:
            if self.imageAvailable == False and self.videoAvailable == False: raise Exception("Must import media file first!") 
    ...
    def runButtonEvent(self):
        try:
            ...
            if self.fileType.currentIndex() == 0 and not self.videoAvailable and self.predictionThreadFinished: raise Exception("No Video Selected")
            if self.fileType.currentIndex() == 1 and not self.imageAvailable and self.predictionThreadFinished: raise Exception("No Image Selected")
    ...
    def runPrediction(self):
        try:
            ...
            if self.fileType.currentIndex() == 1:
                ...
                filename = "{}_detection.png".format(image_file_name)
                file_path = os.path.join(self.project_path, filename)
                frame = cv2.imread(self.image_source)
                total_frames = 1
                isImage = True
            else:
                if self.videoAvailable == False: raise Exception("No Video Selected")
                video_file_name = os.path.splitext(os.path.basename(self.video_source))[0]
                filename = "{}_detection.mp4".format(video_file_name)
                file_path = os.path.join(self.project_path, filename)
                cap = cv2.VideoCapture(self.video_source)
                ret, frame = cap.read()
    ...
    def setCamera(self, index):
        try:
            ...
            QMessageBox.information(self, "Error setting the camera", f"{e}")
    ...
    def recordStreamEvent(self, fromThread=False):
        try:
            if self.isStreaming == False: raise Exception("Start stream first")
                ...
                self.video_path = rf"{self.streamDirectory_text_edit.toPlainText()}"
                self.video_path = os.path.join(self.project_path, 'recordings')
                ...
                if not self.is_recording:
                    ...
                    file_path = os.path.join(self.video_path, self.videoFilename)
                    ...
                    if not fromThread:
                        QMessageBox.information(self, "Recording Saved", f"{self.videoFilename}")

        except Exception as e:
            if not fromThread:
                QMessageBox.information(self, "Problem encountered with recording", f"{e}")
    ...
    def captureStreamEvent(self):
        try:
            if self.isStreaming == False: raise Exception("Start stream first") 
            self.streamDirectory_text_edit = self.findChild(QtWidgets.QTextEdit, "streamDirectory")
            self.image_path = rf"{self.streamDirectory_text_edit.toPlainText()}"
            if self.image_path == "": self.image_path = self.project_path  #set to default path
            ...
            if ret:
                ...
                if ok and filename: self.capturedFilename = f"{filename}.png"
                    ...
                    return
                file_path = os.path.join(self.image_path, self.capturedFilename)
                image.save(file_path)

                QtWidgets.QMessageBox.information(self, "Saved", f"{self.image_path}")   #Show a message box to confirm the frame capture
        except Exception as e:
            QMessageBox.information(self, "Problem encountered with frame capture", f'{e}')
    ...
    def stopStreamEvent(self, fromThread=False):
        try:
            ...
            if not fromThread:
                QMessageBox.information(self, "Stream Unavailable", f'{e}')
    ...
    def startStreamEvent(self):
        if self.cameraComboBox.currentIndex() == 1: #RTMP mode live streaming (check if available ang connection)
            ...
            try:
                ...
                QMessageBox.information(self, "Error with connection to control server", "Program will try to continue RTMP with general video streams")
